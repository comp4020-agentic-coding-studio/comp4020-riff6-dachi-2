...
export function mountCorridor(root: HTMLElement) {
  ...
  const stop = () => { keys.clear(); cancelAnimationFrame(raf); raf = 0; previous = 0; clearDwell(); };
  const render = () => {
    world.style.transform = `translate3d(${-x}px, 0, ${distance}px)`;
    ...
    if (!busy) prompt.textContent = index < 0 ? 'Walk forward to the next pair of doors.' : `Week ${index + 1} · ${weeks[index].title} — Enter to open`;
  };
  ...
  const enter = (index: number) => {
    ...
    populate(index);
    x = index % 2 ? 155 : -155;
    distance = depth(index) - 175;
    world.style.transition = reduced.matches ? 'none' : 'transform 600ms cubic-bezier(.2,.7,.2,1)';
    render(); frames[index].dataset.open = 'true';
    prompt.textContent = `Opening Week ${index + 1} · ${weeks[index].title}…`;
    arrival = setTimeout(() => {
      if (signal.aborted || current !== token) return;
      world.style.transition = ''; busy = false; portal.showModal(); portalTitle.focus({ preventScroll: true });
  ...
  const tick = (now: number) => {
    if (signal.aborted || reduced.matches || !keys.size || busy || portal.open) { raf = 0; return; }
    ...
    x = Math.max(-235, Math.min(235, x + dx / norm * 260 * dt));
  ...
  viewport.addEventListener('keydown', event => {
    ...
    manual = true; clearDwell(); keys.add(key); world.style.transition = '';
    if (!raf) { previous = 0; raf = requestAnimationFrame(tick); }
  }, { signal });
  viewport.addEventListener('keyup', event => {
    if (event.target !== viewport) return;
    keys.delete(event.key.toLowerCase());
    if (!keys.size) { cancelAnimationFrame(raf); raf = 0; previous = 0; startDwell(); }
  }, { signal });
  ...
  root.querySelectorAll<HTMLAnchorElement>('[data-door], [data-jump]').forEach(anchor => {
    ...
    if (anchor.dataset.door !== undefined) anchor.addEventListener('focus', () => {
      if (busy || portal.open) return;
      x = index % 2 ? 100 : -100; distance = depth(index) - 350; render();
    }, { signal });
  });
  root.querySelector('[data-corridor-reset]')!.addEventListener('click', () => {
    stop(); token++; clearTimeout(arrival); busy = false; manual = false; suppressed = -1;
    frames.forEach(frame => delete frame.dataset.open); x = 0; distance = 120;
  ...
  portal.addEventListener('close', () => {
    token++; clearTimeout(departure); clearTimeout(arrival); busy = false;
    if (opened >= 0) delete frames[opened].dataset.open;
    delete portal.dataset.leaving;
  ...
  reduced.addEventListener('change', () => { stop(); token++; clearTimeout(arrival); busy = false; world.style.transition = ''; frames.forEach(frame => delete frame.dataset.open); render(); }, { signal });
  render(); root.dataset.ready = 'true';
  return () => { controller.abort(); stop(); token++; clearTimeout(arrival); clearTimeout(departure); if (portal.open) portal.close(); };
}
